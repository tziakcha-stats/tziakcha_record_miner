...
void PrintHandtilesFormat() {
    ...
    std::cout << "  Basic tiles: [1-9]+[msp] for number tiles, [ESWNCFP] for honor tiles\n";
    std::cout << "  Melds: [XXX,N] for melded tiles (pung/kong/chow)\n";
    std::cout << "  Situation: |WW0000 format (wind|self-drawn|absolute terminal|sea bottom|kong rob)\n";
...
int main(int argc, char* argv[]) {
    ...
    cxxopts::Options options("fan_calculator", "Mahjong Fan Calculator - GB Mahjong");
    ...
    try {
        ...
        LOG(INFO) << "Parsed handtiles: " << calculator.GetStandardHandtilesString();
        ...
        for (size_t i = 0; i < fan_details.size(); ++i) {
            ...
            if (!detail.pack_descriptions.empty()) {
                LOG(INFO) << "  Fan " << (i + 1) << ": " << detail.fan_name 
                          << " (" << detail.fan_score << " fan) - Packs: "
                          << detail.pack_descriptions.size();
        ...
        LOG(INFO) << "Fan calculation completed successfully. Total: " 
                  << calculator.GetTotalFan() << " fan, "
                  << fan_details.size() << " pattern(s)";
