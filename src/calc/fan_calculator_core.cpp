...
FanCalculator::FanCalculator() 
    : is_parsed_(false), is_calculated_(false) {
    LOG(INFO) << "FanCalculator initialized";
}

FanCalculator::~FanCalculator() {
    LOG(INFO) << "FanCalculator destroyed";
}
...
bool FanCalculator::IsWinningHand() const {
    ...
    LOG(INFO) << "IsWinningHand check result: " << (is_winning ? "true" : "false");
...
std::vector<FanResult> FanCalculator::GetFanDetails() const {
    ...
    for (int i = 1; i < mahjong::FAN_SIZE; i++) {
        ...
        for (size_t j = 0; j < fan_.fan_table_res[i].size(); j++) {
            ...
            LOG(INFO) << "Fan detail: " << result.fan_name 
                      << " (" << result.fan_score << " fan) "
...
std::vector<FanTypeInfo> FanCalculator::GetFanTypesSummary() const {
    ...
    for (int i = 1; i < mahjong::FAN_SIZE; i++) {
        ...
        if (count > 0) {
            ...
            LOG(INFO) << "Fan type: " << info.fan_name 
                      << ", count: " << info.count
